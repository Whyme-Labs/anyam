...
7. If the canonical Project Revision advanced, Landing fails explicitly and does not partially write any Source Space. Rebase updates the Workspace base and publishes a new Change Revision under the same stable Change ID; it does not mutate the old revision or silently repair a stale cohort.
