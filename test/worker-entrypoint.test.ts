...
test("Realm Worker Authority Plane runs an authenticated Project-to-Promotion command path through binding-shaped Durable Object state", async () => {
  ...
  const artifactPayload = { projectId: project.id, artifactId: "artifact:authority-test", type: "cli.archive", digest: "sha256:artifact", projectRevisionId: "candidate:authority-test", changeRevisionId: revision.id, runId: "run:authority-test", actionId: "action:unit", outputPath: "dist/cli.archive", disclosure: { projectionId: "projection:authority-test", classification: "project" } };
  const artifactRest = await record("/api/artifacts", "idem:artifact", artifactPayload);
  assert.equal(artifactRest.response.status, 200, JSON.stringify(artifactRest.value));
  assert.equal(artifactRest.value.status, "succeeded");
  assert.equal(artifactRest.value.credentialFree, true);
  assert.equal(artifactRest.value.canonicalWrite, false);
  assert.equal(((artifactRest.value.artifact as Record<string, unknown>).id), "artifact:authority-test");
  assert.equal(((artifactRest.value.artifact as Record<string, unknown>).outputPath), undefined);
  assert.match(String(artifactRest.value.receipt), /typedSurface=rest/);
  const artifactReplayRest = await record("/api/artifacts", "idem:artifact", artifactPayload);
  assert.deepEqual(artifactReplayRest.value, artifactRest.value);
  const artifactMalformed = await record("/api/artifacts", "idem:artifact-malformed", { ...artifactPayload, unsupported: true });
  assert.equal(artifactMalformed.response.status, 422);
  assert.equal(artifactMalformed.value.code, "invalid_request");
  const artifactHidden = await record("/api/artifacts", "idem:artifact-hidden", { ...artifactPayload, projectId: "project:hidden", artifactId: "artifact:hidden" });
  assert.equal(artifactHidden.response.status, 404);
  assert.equal(artifactHidden.value.code, "artifact_record_not_found");
  assert.equal(JSON.stringify(artifactHidden.value).includes("project:hidden"), false);
  const artifactWrongMethod = await record("/api/artifacts", "idem:artifact-method", artifactPayload, "PUT");
  assert.equal(artifactWrongMethod.response.status, 405);
  assert.equal(artifactWrongMethod.value.code, "method_not_allowed");
  const artifactMismatch = await record("/api/artifacts", "idem:artifact-mismatch", { ...artifactPayload, artifactId: "artifact:authority-mismatch", actionId: "action:other" });
  assert.equal(artifactMismatch.response.status, 409);
  assert.equal(artifactMismatch.value.code, "artifact_record_conflict");
  const landingPayload = { projectId: project.id, changeId: change.id, changeRevisionId: revision.id, projectRevisionId: "candidate:authority-test", expectedCanonicalProjectRevisionId: canonicalBefore };
  const landingStale = await record("/api/landings", "idem:landing-stale", { ...landingPayload, expectedCanonicalProjectRevisionId: "project-revision:stale" });
  assert.equal(landingStale.response.status, 409);
  assert.equal(landingStale.value.code, "landing_apply_conflict");
  const landingRest = await record("/api/landings", "idem:landing", landingPayload);
  assert.equal(landingRest.response.status, 200, JSON.stringify(landingRest.value));
  assert.equal(landingRest.value.status, "succeeded");
  assert.equal(landingRest.value.credentialFree, true);
  assert.equal(landingRest.value.canonicalWrite, "landing-only");
  assert.equal(JSON.stringify(landingRest.value).includes("sourceSpaceSnapshots"), false);
  assert.equal(((landingRest.value.landing as Record<string, unknown>).changeRevisionId), revision.id);
  const landedRevision = ((landingRest.value.canonicalRevision as Record<string, unknown>).id as string);
  assert.notEqual(landedRevision, canonicalBefore);
  const landingReplayRest = await record("/api/landings", "idem:landing", landingPayload);
  assert.deepEqual(landingReplayRest.value, landingRest.value);
  const landingHidden = await record("/api/landings", "idem:landing-hidden", { ...landingPayload, projectId: "project:hidden" });
  assert.equal(landingHidden.response.status, 404);
  assert.equal(landingHidden.value.code, "landing_apply_not_found");
  assert.equal(JSON.stringify(landingHidden.value).includes("project:hidden"), false);
  const landingMismatch = await record("/api/landings", "idem:landing-mismatch", { ...landingPayload, changeId: "change:other" });
  assert.equal(landingMismatch.response.status, 404);
  assert.equal(landingMismatch.value.code, "landing_apply_not_found");
  const landingWrongMethod = await record("/api/landings", "idem:landing-method", landingPayload, "PUT");
  assert.equal(landingWrongMethod.response.status, 405);
  assert.equal(landingWrongMethod.value.code, "method_not_allowed");
  const landingUnknownField = await record("/api/landings", "idem:landing-unknown", { ...landingPayload, unsupported: true });
  assert.equal(landingUnknownField.response.status, 422);
  assert.equal(landingUnknownField.value.code, "invalid_request");
  const landingKeyMismatch = await record("/api/landings", "idem:landing-key-mismatch", { ...landingPayload, idempotencyKey: "body-key" });
  assert.equal(landingKeyMismatch.response.status, 422);
  assert.equal(landingKeyMismatch.value.code, "invalid_request");
  const releasePayload = { projectId: project.id, releaseId: "release:authority-test", projectRevisionId: landedRevision, artifactIds: ["artifact:authority-test"], evidenceIds: ["evidence:authority-test"], policyVersion: "policy:authority-test", configurationDigests: [], stateAssumptions: [], changeRevisionId: revision.id, name: "Authority test release", provenanceDigest: "sha256:release-provenance" };
  const releaseRest = await record("/api/releases", "idem:release", releasePayload);
  assert.equal(releaseRest.response.status, 200, JSON.stringify(releaseRest.value));
  assert.equal(releaseRest.value.status, "succeeded");
  assert.equal(releaseRest.value.credentialFree, true);
  assert.equal(releaseRest.value.canonicalWrite, false);
  assert.equal(((releaseRest.value.release as Record<string, unknown>).projectId), project.id);
  assert.equal(((releaseRest.value.release as Record<string, unknown>).projectRevisionId), landedRevision);
  assert.deepEqual(((releaseRest.value.release as Record<string, unknown>).artifactIds), ["artifact:authority-test"]);
  assert.deepEqual(((releaseRest.value.release as Record<string, unknown>).evidenceIds), ["evidence:authority-test"]);
  assert.equal(((releaseRest.value.release as Record<string, unknown>).configurationDigests), undefined);
  assert.equal(((releaseRest.value.release as Record<string, unknown>).stateAssumptions), undefined);
  assert.equal(((releaseRest.value.release as Record<string, unknown>).provenanceDigest), undefined);
  assert.match(String(releaseRest.value.receipt), /typedSurface=rest/);
  const releaseReplayRest = await record("/api/releases", "idem:release", releasePayload);
  assert.deepEqual(releaseReplayRest.value, releaseRest.value);
  const releaseUnknownField = await record("/api/releases", "idem:release-unknown", { ...releasePayload, unsupported: true });
  assert.equal(releaseUnknownField.response.status, 422);
  assert.equal(releaseUnknownField.value.code, "invalid_request");
  const releaseHidden = await record("/api/releases", "idem:release-hidden", { ...releasePayload, projectId: "project:hidden" });
  assert.equal(releaseHidden.response.status, 404);
  assert.equal(releaseHidden.value.code, "release_create_not_found");
  assert.equal(JSON.stringify(releaseHidden.value).includes("project:hidden"), false);
  const releaseArtifactMismatch = await record("/api/releases", "idem:release-artifact-mismatch", { ...releasePayload, releaseId: "release:authority-artifact-mismatch", projectRevisionId: canonicalBefore });
  assert.equal(releaseArtifactMismatch.response.status, 409);
  assert.equal(releaseArtifactMismatch.value.code, "release_create_conflict");
  const releaseChangeMismatch = await record("/api/releases", "idem:release-change-mismatch", { ...releasePayload, releaseId: "release:authority-change-mismatch", changeRevisionId: "change-revision:other" });
  assert.equal(releaseChangeMismatch.response.status, 409);
  assert.equal(releaseChangeMismatch.value.code, "release_create_conflict");
  const releaseStale = await record("/api/releases", "idem:release-stale", { ...releasePayload, releaseId: "release:authority-stale", expectedVersion: 0 });
  assert.equal(releaseStale.response.status, 409);
  assert.equal(releaseStale.value.code, "release_create_conflict");
  const releaseWrongMethod = await record("/api/releases", "idem:release-method", releasePayload, "PUT");
  assert.equal(releaseWrongMethod.response.status, 405);
  assert.equal(releaseWrongMethod.value.code, "method_not_allowed");
  const releaseKeyMismatch = await record("/api/releases", "idem:release-key-mismatch", { ...releasePayload, idempotencyKey: "body-key" });
  assert.equal(releaseKeyMismatch.response.status, 422);
  assert.equal(releaseKeyMismatch.value.code, "invalid_request");
  const targetPayload = { targetId: "target:authority-test", projectId: project.id, name: "test target", adapterId: "target:unqualified", acceptedArtifactTypes: ["cli.archive"], requiredEvidenceKeys: [] };
  const targetRest = await record("/api/targets", "idem:target", targetPayload);
  assert.equal(targetRest.response.status, 200, JSON.stringify(targetRest.value));
  assert.equal(targetRest.value.status, "succeeded");
  assert.equal(targetRest.value.credentialFree, true);
  assert.equal(targetRest.value.canonicalWrite, false);
  assert.equal(((targetRest.value.target as Record<string, unknown>).id), "target:authority-test");
  assert.equal(((targetRest.value.target as Record<string, unknown>).projectId), project.id);
  assert.deepEqual(((targetRest.value.target as Record<string, unknown>).acceptedArtifactTypes), ["cli.archive"]);
  assert.deepEqual(((targetRest.value.target as Record<string, unknown>).requiredEvidenceKeys), []);
  assert.equal(JSON.stringify(targetRest.value).includes("actor"), false);
  assert.equal(JSON.stringify(targetRest.value).includes("qualification"), false);
  assert.match(String(targetRest.value.receipt), /typedSurface=rest/);
  const targetReplayRest = await record("/api/targets", "idem:target", targetPayload);
  assert.deepEqual(targetReplayRest.value, targetRest.value);
  const targetUnknownField = await record("/api/targets", "idem:target-unknown", { ...targetPayload, unsupported: true });
  assert.equal(targetUnknownField.response.status, 422);
  assert.equal(targetUnknownField.value.code, "invalid_request");
  const targetHidden = await record("/api/targets", "idem:target-hidden", { ...targetPayload, targetId: "target:hidden", projectId: "project:hidden" });
  assert.equal(targetHidden.response.status, 404);
  assert.equal(targetHidden.value.code, "target_configure_not_found");
  assert.equal(JSON.stringify(targetHidden.value).includes("project:hidden"), false);
  const targetDuplicate = await record("/api/targets", "idem:target-duplicate", targetPayload);
  assert.equal(targetDuplicate.response.status, 409);
  assert.equal(targetDuplicate.value.code, "target_configure_conflict");
  const targetStale = await record("/api/targets", "idem:target-stale", { ...targetPayload, targetId: "target:stale", expectedVersion: 0 });
  assert.equal(targetStale.response.status, 409);
  assert.equal(targetStale.value.code, "target_configure_conflict");
  const targetWrongMethod = await record("/api/targets", "idem:target-method", targetPayload, "PUT");
  assert.equal(targetWrongMethod.response.status, 405);
  assert.equal(targetWrongMethod.value.code, "method_not_allowed");
  const targetKeyMismatch = await record("/api/targets", "idem:target-key-mismatch", { ...targetPayload, idempotencyKey: "body-key" });
  assert.equal(targetKeyMismatch.response.status, 422);
  assert.equal(targetKeyMismatch.value.code, "invalid_request");
  ...
  assert.equal(promotionRest.value.credentialFree, true);
  assert.equal(promotionRest.value.canonicalWrite, false);
  const promotionProjection = promotionRest.value.promotion as Record<string, unknown>;
  assert.equal(promotionProjection.id, "promotion:authority-rest");
  ...
  assert.equal(JSON.stringify(promotionRest.value).includes("actor"), false);
  assert.equal(JSON.stringify(promotionRest.value).includes("session"), false);
  assert.equal(JSON.stringify(promotionRest.value).includes("providerAdapter"), false);
  assert.match(String(promotionRest.value.receipt), /providerExecution=not-performed/);
  const promotionReplayRest = await record("/api/promotions", "idem:promotion-rest", promotionPayload);
  assert.deepEqual(promotionReplayRest.value, promotionRest.value);
  const promotionUnknownField = await record("/api/promotions", "idem:promotion-unknown", { ...promotionPayload, unsupported: true });
  assert.equal(promotionUnknownField.response.status, 422);
  assert.equal(promotionUnknownField.value.code, "invalid_request");
  const promotionHidden = await record("/api/promotions", "idem:promotion-hidden", { ...promotionPayload, projectId: "project:hidden", promotionId: "promotion:hidden" });
  ...
  const promotionWrongMethod = await record("/api/promotions", "idem:promotion-method", promotionPayload, "PUT");
  assert.equal(promotionWrongMethod.response.status, 405);
  assert.equal(promotionWrongMethod.value.code, "method_not_allowed");
  const promotionKeyMismatch = await record("/api/promotions", "idem:promotion-key-mismatch", { ...promotionPayload, idempotencyKey: "body-key" });
  assert.equal(promotionKeyMismatch.response.status, 422);
  assert.equal(promotionKeyMismatch.value.code, "invalid_request");
  const promotionMalformedPath = await record("/api/promotions/extra", "idem:promotion-path", promotionPayload);
  assert.equal(promotionMalformedPath.response.status, 400);
  assert.equal(promotionMalformedPath.value.code, "invalid_promotion_path");
  const promotionResult = await command("promotion.request", "idem:promotion", { projectId: project.id, releaseId: "release:authority-test", targetId: "target:authority-test" });
