...
IDs, workflow path and SHA, ref, run, audience, and replay identity. The
operation set is explicit: `inbound` initialization, `proposal` Change intake,
or `outbound` projection.
...
GitHub remains an external projection; Anyam Landing remains the only
