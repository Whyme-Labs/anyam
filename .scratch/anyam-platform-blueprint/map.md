...
> Migrated to [GitHub issue #1](https://github.com/Whyme-Labs/anyam/issues/1). GitHub is the authoritative tracker; these local files are retained only as the bootstrap snapshot.
...
- The issue tracker for this effort is GitHub Issues in `Whyme-Labs/anyam`.

## Decisions so far

<!-- Resolved tickets are indexed here. The detailed decision lives only in the ticket. -->

- [Verify the current platform and standards assumptions](https://github.com/Whyme-Labs/anyam/issues/2) — Cloudflare is viable for Anyam's control plane and bounded Linux execution, with mandatory provider abstractions, an Anyam-owned Realm authorization layer, and explicit qualification gates for Artifacts and the current MCP stack.
