...
async function qualifyCustomerRealmAuthority(input: {
  ...
  const createdWorkspace = await client.createWorkspace(projectId, { workspaceId, projectRevisionId, sourceSpaceIds: [sourceSpaceId], mounts: [], projectionId, classification: "public" }, `github-app:${input.qualificationId}:authority:workspace-create`);
