...
- `initialize`
- `tools/list`
- `tools/call` for `project.inspect`
...
`test/realm-mcp.test.ts` covers initialization, tool discovery, project-scoped
inspection, authenticated coordinator binding, malformed JSON-RPC, unknown
methods, mutation denial, missing-project concealment, missing scope, and
